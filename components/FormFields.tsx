...
export function UrlInput({ value, onChange, placeholder = 'yourbrand.com', required }: UrlInputProps) {
  const handleChange = (raw: string) => {
    let v = raw.trim()
    v = v.replace(/^(https?:\/\/)+/, 'https://')
    v = v.replace(/^http:\/\//, 'https://')
    if (v === '') { onChange(''); return }
    if (!v.startsWith('https://')) v = 'https://' + v
    onChange(v)
  }
  return (
    <input type="url" value={value}
      onChange={e => handleChange(e.target.value)}
      onFocus={() => { if (!value) onChange('https://') }}
      onBlur={() => { if (value === 'https://') onChange('') }}
      placeholder={`https://${placeholder}`}
      required={required}
      style={{ background: 'var(--surface2)', border: '0.5px solid var(--border2)', borderRadius: 7, color: 'var(--text-primary)', fontSize: 13, fontFamily: 'inherit', padding: '8px 12px', width: '100%', outline: 'none' }}
    />
  )
}