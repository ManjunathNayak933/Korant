...
interface Affiliate {
  ...
  commission_value: number; commission_trigger: string; created_at: string; campaign_id?: string
}
interface Program {
  id: string; name: string; commission_type: string; commission_value: number
...
export default function AffiliateDashboard({ clientId, campaigns, baseUrl, month, onCampaignAdd }: Props) {
  ...
  return (
    <div>
      ...
      {/* 10-KPI channel stats bar */}
      <ChannelStatsBar clientId={clientId} channel="affiliate" campaignId={selectedCampaign} month={month} />

      {/* Visitor breakdown per partner */}
      <AssetInsights clientId={clientId} month={month} channel="affiliate" />

      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 12 }}>
          {[1,2,3].map(i => <div key={i} style={{ height: 160, background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10, opacity: 0.4 }} />)}
        </div>
      ) : (
        <>
          {/* ── AFFILIATES ── */}
                    ...
                    return (
                      <div key={aff.id} style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10, padding: 16, opacity: aff.is_active ? 1 : 0.5 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button onClick={() => setStatsModal(aff)} style={{ flex: 1, background: 'transparent', border: '0.5px solid var(--border2)', color: 'var(--text-muted)', borderRadius: 5, padding: '5px 0', fontSize: 11, cursor: 'pointer' }}>Full stats</button>
                ...
                <div key={prog.id} style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 8, padding: 14, marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>
                      {prog.commission_value}{prog.commission_type === 'percentage' ? '%' : '₹'} · {prog.commission_trigger.replace('_', ' ')} · {prog.attribution_window_days}d window · {affiliates.filter(a => (a as any).program_id === prog.id).length} affiliates
                    </div>
                  </div>
                  <button onClick={async () => {
            ...
            <div>
              ...
              {filteredAmbassadors.length === 0 ? (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      {['Name','Handle','Email','Joined','Status','Actions'].map(h => (
                        <th key={h} style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-dim)', padding: '8px 12px', textAlign: 'left', borderBottom: '0.5px solid var(--border3)', fontWeight: 400 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
        ...
        return (
          <Modal title={`${statsModal.name} — Full stats`} onClose={() => setStatsModal(null)}>
            ...
            {v ? (
              ...
              <div style={{ padding: '10px 0', fontSize: 11, color: 'var(--text-dim)' }}>Install beacon on your store to see visitor return/share data for this affiliate.</div>
            )}
          </Modal>
        )
      })()}
